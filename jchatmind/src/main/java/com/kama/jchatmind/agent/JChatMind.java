...
public class JChatMind {
    ...
    private void execute() {
        ...
        this.chatMemory.add(this.chatSessionId, safeWindow(toolExecutionResult.conversationHistory()));
    ...
    public void run() {
        ...
        try {
            ...
            agentState = AgentState.FINISHED;
            // 仅当最后一步有工具调用且没有文字输出时才补充总结。
            // 若 think() 最后一步已同时输出了文字（即使也有工具调用），则无需重复生成。
            if (lastChatResponse != null && lastChatResponse.hasToolCalls()) {
                String lastText = lastChatResponse.getResult().getOutput().getText();
    ...
    /**
     * 所有工具调用完成后，发起一次无工具的对话，让 AI 生成纯文字总结。
     * GLM-4.6 等模型在调用工具时不输出文字，需要额外一步才能产生用户可见的回复。
     */
    private void generateFinalSummary() {
        try {
            ...
            ChatResponse response = this.chatClient
                    .prompt(prompt)
                    .system("你已经完成了所有工具调用。请根据上述执行结果，用简洁友好的语言向用户呈现最终结论。不要再调用任何工具。")
                    .call()
                    .chatClientResponse()
                    .chatResponse();
            ...
            log.warn("生成最终总结失败（不影响主流程）: {}", e.getMessage());
        }
    }

    /**
     * 安全截断消息历史：确保截断后首条消息不是孤立的 ToolResponseMessage。
     * MessageWindowChatMemory 按数量截断时可能把 AssistantMessage(toolCalls)
     * 和对应的 ToolResponseMessage 拆开，导致模型 API 报参数非法。
     * 此方法在截断后向前滚动，直到首条消息是 SystemMessage / HumanMessage。
     */
    private List<Message> safeWindow(List<Message> history) {
        if (history == null || history.isEmpty()) {
            return history;
        }
        int start = 0;
        // 跳过开头孤立的 AssistantMessage(含 toolCalls) 或 ToolResponseMessage
        while (start < history.size()) {
            Message first = history.get(start);
            if (first instanceof ToolResponseMessage) {
                start++;
            } else if (first instanceof AssistantMessage am && !am.getToolCalls().isEmpty()) {
                start++;
            } else {
                break;
            }
        }
        return start == 0 ? history : new ArrayList<>(history.subList(start, history.size()));
    }
