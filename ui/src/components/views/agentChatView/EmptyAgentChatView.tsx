          ...
          <Sender
            onSubmit={async () => {
              ...
              const response = await createChatSession({
                ...
                title: pendingMessage.slice(0, 20),
              });
              await refreshChatSessions();
              // 先导航建立 SSE 连接，消息通过 state 传给 AgentChatView
              // AgentChatView 收到 SSE init 事件后才发送消息，避免 "No client found" 报错
              navigate(`/chat/${response.chatSessionId}`, {
                state: {
                  pendingMessage: pendingMessage,
                  pendingAgentId: effectiveAgentId,
                },
              });
            }}
            value={message}
